"""
lab_1a.py

The first lab in the BWSI CSS course. To complete this lab, fill out the variable on line 10
with your name. Then, save the code, add it to the staging area, and commit it to the Git tree.

robot_speed = 5 # m/s
"""

def main():
    print("Hello World!")

    name = "Arnav Dusad" # TODO: Insert your name between the double quotes

    print(f"{name}, Welcome to the CSS course!")
    print("Hello everyone! My name is Arnav, and I am a freshman from Virginia. I enjoy learning CS and am looking forward to using GitHub through this course!")

if __name__ == "__main__":
    main()
